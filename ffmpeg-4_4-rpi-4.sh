...
# ./ffmpeg-4_4-rpi-4.sh
# or
# curl -L "https://git.io/JTlcX" | bash
...
VER=2.9.3; curl -LsS "https://github.com/sekrit-twc/zimg/archive/release-${VER}.tar.gz" | tar -C "${TMPDIR}" -xvzf- && cd "${TMPDIR}/zimg-release-${VER}" \
    && ./autogen.sh \
    && ./configure \
    && make -j$(nproc) \
    && sudo make install
...
# Compile FFmpeg
VER=4.4; curl -LsS "https://ffmpeg.org/releases/ffmpeg-${VER}.tar.xz" | tar -C "${TMPDIR}" -xvJf- && cd "${TMPDIR}/ffmpeg-${VER}" \
    && ./configure \
